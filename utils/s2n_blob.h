...
CONTRACT_REQUIRES(b != NULL && __CPROVER_is_fresh(b, sizeof(s2n_blob)))
CONTRACT_REQUIRES(size == 0 || data != NULL)
CONTRACT_ENSURES(s2n_blob_validate(b) == S2N_RESULT_OK);
...
#define s2n_stack_blob(name, requested_size, maximum)   \
    ...
    s2n_blob_init_partial(&name, name##_buf, name##_requested_size)
