...
S2N_RESULT s2n_offered_psk_list_read_next(struct s2n_offered_psk_list *psk_list, struct s2n_offered_psk *psk)
{
    ...
    s2n_blob_init_partial(&psk->identity, identity_data, identity_size);
        ...
        struct s2n_blob *output_binder)
{
    ...
    s2n_blob_init_partial(&psk_keys.extract_secret, psk->early_secret.data, psk_keys.size);
        ...
        const struct s2n_blob *binder_hash, struct s2n_stuffer *out)
{
    ...
    s2n_blob_init_partial(&binder, binder_data, binder_hash->size);
        ...
        struct s2n_stuffer *out)
{
    ...
    for (size_t i = 0; i < psk_list->len; i++) {
        ...
        if (binder_hash->size == 0) {
            ...
            s2n_blob_init_partial(binder_hash, binder_hashes_data[psk->hmac_alg], hash_size);
...
S2N_RESULT s2n_finish_psk_extension(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&partial_client_hello, client_hello->blob.data,
