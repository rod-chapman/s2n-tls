...
int s2n_tls13_client_finished_recv(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&wire_finished_mac, s2n_stuffer_raw_read(&conn->handshake.io, length), length);
    ...
    s2n_blob_init_partial(&finished_key, conn->handshake.client_finished, keys.size);
