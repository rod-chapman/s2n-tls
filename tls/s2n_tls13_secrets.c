...
static S2N_RESULT s2n_calculate_transcript_digest(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&digest, CONN_HASHES(conn)->transcript_hash_digest, digest_size);
...
static S2N_RESULT s2n_extract_handshake_secret(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&derived_secret, derived_secret_bytes, S2N_TLS13_SECRET_MAX_LEN);
        ...
        uint8_t *output_in, uint32_t output_length)
{
    ...
    struct s2n_blob digest = { 0 };
    POSIX_ENSURE_LTE(s2n_get_hash_len(CONN_HMAC_ALG(conn)), S2N_MAX_DIGEST_LEN);
    s2n_blob_init_partial(&digest, digest_bytes, s2n_get_hash_len(CONN_HMAC_ALG(conn)));
    ...
    s2n_blob_init_partial(&output, output_in, output_length);
