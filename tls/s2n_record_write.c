        ...
        struct s2n_blob *plaintext, struct s2n_stuffer *out, uint32_t *bytes_written)
{
    ...
    *bytes_written = 0;
    ...
    s2n_blob_init_partial(&content_length_blob, content_length_bytes, sizeof(content_length_bytes));
...
int s2n_record_writev(struct s2n_connection *conn, uint8_t content_type, const struct iovec *in, int in_count, size_t offs, size_t to_write)
{
    ...
    s2n_blob_init_partial(&record_blob,
    ...
    if (cipher_suite->record_alg->cipher->type == S2N_AEAD) {
        ...
        s2n_blob_init_partial(&iv, aad_iv, sizeof(aad_iv));
        ...
        s2n_blob_init_partial(&iv, implicit_iv, block_size);
        ...
        if (conn->actual_protocol_version > S2N_TLS10) {
            ...
            if (cipher_suite->record_alg->cipher->type == S2N_COMPOSITE) {
                ...
                s2n_blob_init_partial(&explicit_iv_placeholder, zero_block, block_size);
    ...
    s2n_blob_init_partial(&plaintext_blob, orig_write_ptr, data_bytes_to_take);
    ...
    s2n_blob_init_partial(&seq, sequence_number, S2N_TLS_SEQUENCE_NUM_LEN);
