        ...
        uint64_t intro_time_in_seconds_from_epoch)
{
    ...
    s2n_blob_init_partial(&out_key, output_pad, s2n_array_len(output_pad));
    struct s2n_blob in_key = { 0 };
    s2n_blob_init_partial(&in_key, key, key_len);
    struct s2n_blob salt = { 0 };
    s2n_blob_init_partial(&salt, NULL, 0);
    struct s2n_blob info = { 0 };
    s2n_blob_init_partial(&info, NULL, 0);
