...
int s2n_server_hello_send(struct s2n_connection *conn)
{
    ...
    struct s2n_blob b = { 0 };
    s2n_blob_init_partial(&b, conn->handshake_params.server_random, S2N_TLS_RANDOM_DATA_LEN);

    /* Create the server random data */
    POSIX_GUARD(s2n_stuffer_init(&server_random, &b));

    struct s2n_blob rand_data = { 0 };
    s2n_blob_init_partial(&rand_data, s2n_stuffer_raw_write(&server_random, S2N_TLS_RANDOM_DATA_LEN), S2N_TLS_RANDOM_DATA_LEN);
