...
int s2n_update_application_traffic_keys(struct s2n_connection *conn, s2n_mode mode, keyupdate_status status)
{
    ...
    if (mode == S2N_CLIENT) {
        ...
        s2n_blob_init_partial(&old_app_secret, conn->secrets.version.tls13.client_app_secret, keys.size);
        s2n_blob_init_partial(&app_iv, conn->secure->client_implicit_iv, S2N_TLS13_FIXED_IV_LEN);
    } else {
        old_key = &conn->secure->server_key;
        s2n_blob_init_partial(&old_app_secret, conn->secrets.version.tls13.server_app_secret, keys.size);
        s2n_blob_init_partial(&app_iv, conn->secure->server_implicit_iv, S2N_TLS13_FIXED_IV_LEN);
    }
