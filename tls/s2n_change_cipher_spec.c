...
int s2n_client_ccs_recv(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&seq, conn->secure->client_sequence_number, sizeof(conn->secure->client_sequence_number));
...
int s2n_server_ccs_recv(struct s2n_connection *conn)
{
    ...
    /* Zero the sequence number */
    struct s2n_blob seq = { 0 };
    s2n_blob_init_partial(&seq, conn->secure->server_sequence_number, sizeof(conn->secure->server_sequence_number));
    POSIX_GUARD(s2n_blob_zero(&seq));
