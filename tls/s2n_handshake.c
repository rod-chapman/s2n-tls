...
int s2n_conn_find_name_matching_certs(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&hostname_blob, (uint8_t *) (uintptr_t) name, strlen(name));
    ...
    s2n_blob_init_partial(&normalized_name, (uint8_t *) normalized_hostname, hostname_blob.size);
    ...
    if (!conn->handshake_params.exact_sni_match_exists) {
        ...
        s2n_blob_init_partial(&wildcard_blob, (uint8_t *) wildcard_hostname, sizeof(wildcard_hostname));
