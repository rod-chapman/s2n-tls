...
static S2N_RESULT s2n_generate_client_session_id(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&session_id, conn->session_id, S2N_TLS_SESSION_ID_MAX_LEN);
        ...
        uint8_t client_random[S2N_TLS_RANDOM_DATA_LEN])
{
    ...
    s2n_blob_init_partial(&client_hello->session_id, session_id, session_id_len);
    ...
    s2n_blob_init_partial(&client_hello->cipher_suites, cipher_suites, cipher_suites_length);
    ...
    s2n_blob_init_partial(&client_hello->compression_methods, compression_methods, compression_methods_len);
...
int s2n_client_hello_send(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&client_random, conn->handshake_params.client_random, S2N_TLS_RANDOM_DATA_LEN);
    if (!s2n_is_hello_retry_handshake(conn)) {
...
int s2n_sslv2_client_hello_recv(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&client_hello->session_id, s2n_stuffer_raw_read(in, session_id_length), session_id_length);
    ...
    struct s2n_blob b = { 0 };
    s2n_blob_init_partial(&b, conn->handshake_params.client_random, S2N_TLS_RANDOM_DATA_LEN);
    ...
    *extension = (struct s2n_blob){ 0 };
    ...
    while (s2n_stuffer_data_available(&raw_extensions_stuffer) > 0) {
        ...
        if (extension_iana == extension_type) {
            s2n_blob_init_partial(extension, extension_data, extension_size);
            return S2N_RESULT_OK;
        }
    }
    return S2N_RESULT_OK;
