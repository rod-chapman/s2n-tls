...
int s2n_server_key_recv(struct s2n_connection *conn)
{
    ...
    s2n_blob_init_partial(&signature, s2n_stuffer_raw_read(in, signature_length), signature_length);
        ...
        struct s2n_kex_raw_server_data *raw_server_data)
{
    ...
    s2n_blob_init_partial(&kem_id_blob, kem_id_arr, s2n_array_len(kem_id_arr));
