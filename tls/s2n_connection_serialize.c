...
int s2n_connection_serialize(struct s2n_connection *conn, uint8_t *buffer, uint32_t buffer_length)
{
    ...
    s2n_blob_init_partial(&context_blob, buffer, buffer_length);
...
static S2N_RESULT s2n_initialize_implicit_iv(struct s2n_connection *conn, struct s2n_connection_deserialize *parsed_values)
{
    ...
    s2n_blob_init_partial(&seq_num_blob, seq_num, S2N_TLS_SEQUENCE_NUM_LEN);
    ...
    s2n_blob_init_partial(&in_blob, in_data, sizeof(in_data));

    struct s2n_blob iv_blob = { 0 };
    s2n_blob_init_partial(&iv_blob, implicit_iv, S2N_TLS13_FIXED_IV_LEN);

    struct s2n_blob aad_blob = { 0 };
    s2n_blob_init_partial(&aad_blob, NULL, 0);
